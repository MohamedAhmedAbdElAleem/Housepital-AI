...
const classifyMessage = (message) => {
	...
	if (
		...
		msgLower.includes("شديد") ||
		msgLower.includes("high fever") ||
		msgLower.includes("39")
	) {
		urgency = "High";
	} else if (
		msgLower.includes("minor") ||
		msgLower.includes("بسيط") ||
		msgLower.includes("small")
	) {
		urgency = "Low";
	}
	...
	if (matchedServices.length > 0) {
		const servicesList = matchedServices
			.map((s) => `• ${SERVICES[s].title}: ${SERVICES[s].price}`)
			.join("\n");
...
رد بالـ JSON format ده بالظبط:
{
    ...
    "showSos": true أو false
				...
				const serviceRoutes = fallback.services
					.map((s) => SERVICES[s])
					.filter(Boolean);
...
router.post("/analyze-image", async (req, res) => {
	try {
		...
		if (!cvResult || !cvResult.final_verdict) {
			return res
				.status(400)
				.json({ error: "CV result with final_verdict is required" });
		}
		...
		if (finalVerdict.toLowerCase().includes("wound detected")) {
			...
			if (woundType === "unknown") {
				for (const [type, info] of Object.entries(WOUND_TYPE_SERVICES)) {
					if (
						finalVerdict.toLowerCase().includes(type) ||
			...
			if (!dfuGrade) {
				for (const grade of Object.keys(DFU_GRADE_URGENCY)) {
					if (
						finalVerdict.toLowerCase().includes(grade) ||
    ...
    "showSos": ${showSos}
					...
					if (woundType !== "unknown") {
						...
						if (dfuGrade) {
							...
							if (urgency === "Emergency") {
								response =
									`🚨 **حالة طوارئ - قدم سكري ${gradeInfo.description}**\n\n` +
