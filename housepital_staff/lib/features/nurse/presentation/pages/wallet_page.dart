...
class _NurseWalletPageState extends State<NurseWalletPage>
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      ...
                      if (_selectedPaymentMethod == 'wallet') {
                        ...
                        Navigator.pop(ctx);
                        context.read<NurseWalletCubit>().initiateWalletRecharge(amount, phone);
                      } else {
                        Navigator.pop(ctx);
                        context.read<NurseWalletCubit>().initiateCardRecharge(amount);
                      }
                    },
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => context.read<NurseWalletCubit>().loadWallet(),
          ),
        ],
      ),
      body: BlocConsumer<NurseWalletCubit, WalletState>(
        listener: (context, state) {
          ...
          // Card flow → open WebView
          if (state is WalletRechargeCard) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => PaymobWebViewPage(iframeUrl: state.iframeUrl, amount: state.amount),
              ),
            ).then((success) {
              if (success == true) {
                context.read<NurseWalletCubit>().refreshAfterRecharge();
              } else {
                context.read<NurseWalletCubit>().loadWallet();
              }
            });
          ...
          // Wallet flow → open redirect URL in external browser/app
          if (state is WalletRechargeWallet) {
        ...
        builder: (context, state) {
          ...
          return const Center(child: Text('Something went wrong', style: TextStyle(color: AppColors.textSecondary)));
        },
      ),
    );
  }
  ...
  Future<void> _openWalletRedirect(String url, double amount) async {
    ...
    try {
      ...
      if (!launched) {
        ...
        // Show dialog to confirm after returning from wallet app
        if (!mounted) return;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            state.walletBlockReason ?? 'Your wallet balance has exceeded the minimum threshold. You cannot receive new orders.',
  ...
  Widget _buildWarningBanner(WalletLoaded state) {
    return Container(
      padding: const EdgeInsets.all(16),
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Your balance is negative. If it drops below ${state.threshold.toStringAsFixed(0)} EGP, you will not be able to receive new orders.',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'A ${(state.commissionRate * 100).toStringAsFixed(0)}% platform commission is deducted from your wallet for each completed service. You collect the full cash payment from patients.',
  ...
  Widget _buildTransactionTile(dynamic tx) {
    ...
    switch (type) {
      ...
      case 'nurse_earning': icon = Icons.medical_services_rounded; break;
