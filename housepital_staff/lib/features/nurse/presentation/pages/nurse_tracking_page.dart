...
class _NurseTrackingPageState extends State<NurseTrackingPage>
  ...
  Future<void> _updateLocationOnServer(double lat, double lng) async {
    try {
      ...
      final bookingId =
          context.read<NurseBookingCubit>().currentBooking?.id ??
          widget.booking.id;
  ...
  Future<void> _fetchRoute() async {
    ...
    debugPrint(
      '🗺️ NURSE ROUTE DEBUG: currentLocation=$_currentLocation, patientLocation=$_patientLocation',
    );
    if (_currentLocation != null && _patientLocation != null) {
      ...
      try {
        ...
        debugPrint(
          '🗺️ NURSE ROUTE DEBUG: OSRM status = ${response.statusCode}',
        );
        if (response.statusCode == 200) {
          ...
          if (data['routes'] != null && data['routes'].isNotEmpty) {
            ...
            debugPrint(
              '🗺️ NURSE ROUTE DEBUG: Got ${coords.length} route points, distance=${route['distance']}, duration=${route['duration']}',
            );
            if (mounted) {
              setState(() {
            ...
            debugPrint(
              '🗺️ NURSE ROUTE DEBUG: No routes in response: ${response.body.substring(0, 200)}',
            );
          }
        } else {
          debugPrint(
            '🗺️ NURSE ROUTE DEBUG: OSRM error: ${response.body.substring(0, 200)}',
          );
        }
      } catch (e) {
  ...
  void _updateStatus(String newStatus) {
    ...
    final realId =
        context.read<NurseBookingCubit>().currentBooking?.id ??
        widget.booking.id;
    context.read<NurseBookingCubit>().updateBookingStatus(realId, newStatus);
  }
  ...
  Widget _buildActionButton(String status) {
    ...
    if (status == 'assigned') {
      ...
      text = l10n.iveArrived;
      color = AppColors.warning;
      icon = Icons.location_on_rounded;
      onPressed = () => _updateStatus('arrived');
    } else {
      text = l10n.enterPatientPin;
      color = AppColors.success;
      icon = Icons.pin_rounded;
      onPressed = () {
        final cubitBooking = context.read<NurseBookingCubit>().currentBooking;
        final bookingForPin = cubitBooking ?? widget.booking;
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => PinVerificationPage(booking: bookingForPin),
          ),
        );
      };
    }
