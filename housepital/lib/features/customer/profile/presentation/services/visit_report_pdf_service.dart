...
import 'visit_report_models.dart';
...
class VisitReportPdfService {
  ...
  Future<void> share(
    ...
    final nurseName = record['nurseId']?['user']?['name'] ?? 'Care Provider';
    final patientName = booking['patientName'] ?? 'Patient';
    final visitDate = record['createdAt'] != null ? DateTime.parse(record['createdAt']) : DateTime.now();

    final doc = await _build(record, report, nurseName, patientName, visitDate);
    await Printing.sharePdf(
      bytes: await doc.save(),
      filename: 'VisitReport_${patientName.replaceAll(' ', '_')}_${DateFormat('yyyyMMdd').format(visitDate)}.pdf',
    );
  }
  ...
  Future<void> preview(
    ...
    await Printing.layoutPdf(
      ...
      name: 'VisitReport_${patientName.replaceAll(' ', '_')}_${DateFormat('yyyyMMdd').format(visitDate)}.pdf',
    );
  }
  ...
  Future<pw.Document> _build(
    ...
    final doc = pw.Document(
      title: 'Visit Report – $patientName',
      author: nurseName,
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        header: (ctx) => _pageHeader(ctx, record, nurseName, patientName, visitStart, fontBold, fontSemiBold),
        ...
        build: (ctx) => [
          ...
          pw.SizedBox(height: 16),
          _sectionCareProvided(report, font, fontBold, fontSemiBold),
          pw.SizedBox(height: 16),
            ...
            children: [
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  children: [
                    ...
                    _metaSmall(booking['serviceName'] ?? 'Home Nursing'),
                  ],
                ),
              ),
              _vertDivider(),
              pw.Column(
                ...
                children: [
                  ...
                  _metaSmall(
                    booking['_id'] != null && booking['_id'].toString().length > 10
                        ? '...${booking['_id'].toString().substring(booking['_id'].toString().length - 8)}'
                        : booking['_id']?.toString() ?? 'N/A',
                  ),
                ],
              ),
            ],
